...
void output::DoPathLoss(char *filename, unsigned char geo, unsigned char kml,
...
void output::DoSigStr(char *filename, unsigned char geo, unsigned char kml,
	      unsigned char ngs, struct site *xmtr, unsigned char txsites)
{
	/* This function generates a topographic map in Portable Pix Map
	...
	_inputs->LoadSignalColors(xmtr[0]);
...
void output::DoRxdPwr(char *filename, unsigned char geo, unsigned char kml,
	      unsigned char ngs, struct site *xmtr, unsigned char txsites)
{
	/* This function generates a topographic map in Portable Pix Map
...
void output::DoLOS(char *filename, unsigned char geo, unsigned char kml,
	   unsigned char ngs, struct site *xmtr, unsigned char txsites)
{
	/* This function generates a topographic map in Portable Pix Map
	   (PPM) format based on the signal power level values held in the
...
void output::PathReport(struct site source, struct site destination, char *name,
		char graph_it, int propmodel, int pmenv, double rxGain)
{
	...
	if (metric) {
		fprintf(fd2, "Ground elevation: %.2f meters AMSL\n",
			METERS_PER_FOOT *  _signals->GetElevation(source));
		fprintf(fd2,
			"Antenna height: %.2f meters AGL / %.2f meters AMSL\n",
			METERS_PER_FOOT * source.alt,
			METERS_PER_FOOT * (source.alt + _signals->GetElevation(source)));
	}

	else {
		fprintf(fd2, "Ground elevation: %.2f feet AMSL\n",
			_signals->GetElevation(source));
		fprintf(fd2, "Antenna height: %.2f feet AGL / %.2f feet AMSL\n",
			source.alt, source.alt + _signals->GetElevation(source));
	}

	azimuth = _signals->Azimuth(source, destination);
	angle1 = _signals->ElevationAngle(source, destination);
	angle2 = _signals->ElevationAngle2(source, destination, earthradius);
	...
	if (metric)
		fprintf(fd2, "Distance to %s: %.2f kilometers\n",
			destination.name, KM_PER_MILE *  _signals->Distance(source,
								 destination));

	else
		fprintf(fd2, "Distance to %s: %.2f miles\n", destination.name,
			_signals->Distance(source, destination));
	...
	if (metric) {
		fprintf(fd2, "Ground elevation: %.2f meters AMSL\n",
			METERS_PER_FOOT *  _signals->GetElevation(destination));
		fprintf(fd2,
			"Antenna height: %.2f meters AGL / %.2f meters AMSL\n",
			METERS_PER_FOOT * destination.alt,
			METERS_PER_FOOT * (destination.alt +
				_signals->GetElevation(destination)));
	}
		...
		fprintf(fd2, "Antenna height: %.2f feet AGL / %.2f feet AMSL\n",
			destination.alt,
			destination.alt + _signals->GetElevation(destination));
	}

	if (metric)
		fprintf(fd2, "Distance to %s: %.2f kilometers\n", source.name,
			KM_PER_MILE *  _signals->Distance(source, destination));

	else
		fprintf(fd2, "Distance to %s: %.2f miles\n", source.name,
			_signals->Distance(source, destination));

	azimuth = _signals->Azimuth(destination, source);

	angle1 = _signals->ElevationAngle(destination, source);
	angle2 = _signals->ElevationAngle2(destination, source, earthradius);
	...
	_signals->ObstructionAnalysis(source, destination, LR.frq_mhz, fd2);
	...
	if (graph_it) {
		...
		fprintf(fd,
			...
			destination.name, source.name, _signals->Azimuth(destination,
							       source), 176);
		...
		if (metric)
			fprintf(fd,
				...
				KM_PER_MILE *  _signals->Distance(destination, source));
		else
			fprintf(fd,
				"set xlabel \"Distance Between %s and %s (%.2f miles)\"\n",
				destination.name, source.name,
				_signals->Distance(destination, source));
...
void output::SeriesData(struct site source, struct site destination, char *name,
		unsigned char fresnel_plot, unsigned char normalised)
{
	...
	_signals->ReadPath(destination, source);
	azimuth = _signals->Azimuth(destination, source);
	distance = _signals->Distance(destination, source);
	refangle = _signals->ElevationAngle(destination, source);
	b = _signals->GetElevation(destination) + destination.alt + earthradius;
	...
	if (normalised) {
		ed = _signals->GetElevation(destination);
		es = _signals->GetElevation(source);
	...
	for (x = 0; x < path.length - 1; x++) {
		...
		remote.alt = 0.0;
		terrain = _signals->GetElevation(remote);
		if (x == 0)
			terrain += destination.alt;	/* RX antenna spike */

		a = terrain + earthradius;
		cangle = 5280.0 *  _signals->Distance(destination, remote) / earthradius;
